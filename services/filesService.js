...
async function getFilesData (fileName) {
  ...
  const filesResp = await fetchJson(`${API_URL}/files`)
  ...
  const results = await Promise.all(filesToFetch.map(async (file) => {
    try {
      const csv = await fetchCsv(`${API_URL}/file/${file}`)
...
module.exports = { getFilesData, getFilesList, fetchJson, fetchCsv, parseCsv }
